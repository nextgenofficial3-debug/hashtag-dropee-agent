...
export default function CustomersPage() {
  ...
  const [form, setForm] = useState({ name: "", phone: "", email: "", address: "", notes: "", plus_code: "" });
  ...
  const startEdit = (c: Customer) => {
    setEditId(c.id);
    setForm({ name: c.name, phone: c.phone || "", email: c.email || "", address: c.address || "", notes: c.notes || "", plus_code: c.plus_code || "" });
    setShowForm(true);
  };
  ...
  const resetForm = () => {
    setShowForm(false);
    setEditId(null);
    setForm({ name: "", phone: "", email: "", address: "", notes: "", plus_code: "" });
  };
          ...
          <div key={c.id} className="glass rounded-2xl p-4 space-y-2">
            <div className="flex items-start justify-between">
              ...
              <div className="flex gap-1">
                ...
                <button onClick={() => handleDelete(c.id)} className="p-1.5 rounded-lg bg-destructive/10"><Trash2 className="w-3.5 h-3.5 text-destructive" /></button>
              </div>
            </div>
            ...
            {c.plus_code && <p className="text-[10px] text-primary/70 font-mono">📍 {c.plus_code}</p>}
